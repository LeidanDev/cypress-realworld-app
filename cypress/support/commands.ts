...
// ----------------- Visual Snapshot -----------------
Cypress.Commands.add("visualSnapshot", (maybeName?: string) => {
  // @ts-ignore
  let snapshotTitle = cy.state("runnable").fullTitle();
  if (maybeName) {
    snapshotTitle = `${snapshotTitle} - ${maybeName}`;
...
Cypress.Commands.add(
  "login",
  (username: string, password: string, { rememberUser = false } = {}) => {
    const signinPath = "/signin";
...
Cypress.Commands.add(
  "loginByApi",
  (username: string, password: string = Cypress.env("defaultPassword")) => {
    return cy.request("POST", `${Cypress.env("apiUrl")}/login`, {
      username,
      password,
    });
...
Cypress.Commands.add(
  ...
  ($el: JQuery<HTMLElement>) => {
    ...
    const key = Object.keys($el.get(0)).find((k) =>
...
Cypress.Commands.add("setTransactionAmountRange", (min: number, max: number) => {
  cy.getBySel("transaction-list-filter-amount-range-button").scrollIntoView();
  cy.getBySel("transaction-list-filter-amount-range-button").click({ force: true });
...
Cypress.Commands.add(
  ...
  (username: string, password: string = Cypress.env("defaultPassword")) => {
    ...
    cy.visit("/signin", { log: false }).then(() => log.snapshot("before"));
    ...
    cy.wait("@loginUser").then((loginUser: any) => {
      log.set({
        consoleProps() {
          return { username, password, userId: loginUser.response.body.user.id };
        },
      });
    });
...
Cypress.Commands.add("loginByGoogleApi", () => {
  ...
  cy.task<{ clientSecret: string; refreshToken: string }>(
    "getGoogleCredentials"
  ).then(({ clientSecret, refreshToken }) => {
    cy.request({
      method: "POST",
      url: "https://www.googleapis.com/oauth2/v4/token",
      body: {
